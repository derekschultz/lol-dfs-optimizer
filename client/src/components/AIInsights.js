...
const AIInsights = ({ API_BASE_URL, lineups, playerData, displayNotification, exposureSettings, onUpdateExposures, onGenerateOptimizedLineups, onLineupsUpdated }) => {
  ...
  useEffect(() => {
    if (localLineups && localLineups.length > 0 && playerData && playerData.length > 0 && !isApplying) {
  ...
  const fetchLiveData = async () => {
    ...
    setDataFetchStatus({ phase: 'starting', message: 'Getting cached data...' });
    
    try {
      const AI_SERVICE_URL = 'http://localhost:3002';
      
      setDataFetchStatus({ phase: 'connecting', message: 'Connecting to AI service...' });
      ...
      if (result.success) {
        setDataFetchStatus({ 
          phase: 'completed', 
          message: `✅ Retrieved ${result.data?.matches?.total || 0} matches, ${result.data?.players?.total || 0} player stats from cache`,
          data: result.data
        });
        
        const lastUpdated = result.data.lastUpdated ? new Date(result.data.lastUpdated).toLocaleString() : 'Unknown';
        displayNotification(`✅ Data retrieved from cache! ${result.data?.matches?.total || 0} matches, ${result.data?.players?.total || 0} player stats (Updated: ${lastUpdated})`, 'success');
        ...
        displayNotification('❌ No cached data available. Background collection may still be running.', 'warning');
      }
    } catch (error) {
  ...
  const triggerBackgroundCollection = async () => {
    try {
      ...
      if (result.success) {
        displayNotification('✅ Background collection started! Check back in a few minutes for updated data.', 'success');
      } else {
        displayNotification(`❌ ${result.error || 'Failed to start background collection'}`, 'error');
      }
    } catch (error) {
      displayNotification(`❌ Failed to trigger collection: ${error.message}`, 'error');
    }
  };
  ...
  const checkCollectionStatus = async () => {
    try {
      ...
      const response = await fetch(`${AI_SERVICE_URL}/api/ai/collection-status`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        }
      ...
      displayNotification(`❌ Failed to check status: ${error.message}`, 'error');
    }
  };
    ...
    <div className="card">
      ...
      {dataFetchStatus && (
        <div style={{
          ...
          borderRadius: '8px',
          border: '1px solid',
          borderColor: dataFetchStatus.phase === 'error' ? '#fecaca' : 
                       dataFetchStatus.phase === 'completed' ? '#d1fae5' : '#dbeafe',
          backgroundColor: dataFetchStatus.phase === 'error' ? '#fef2f2' : 
                          dataFetchStatus.phase === 'completed' ? '#ecfdf5' : '#eff6ff',
          display: 'flex',
          alignItems: 'center',
          gap: '12px'
        }}>
          {dataFetchStatus.phase !== 'completed' && dataFetchStatus.phase !== 'error' && (
            <div style={{
              width: '16px',
              height: '16px',
          ...
          <div style={{ flex: 1 }}>
            <div style={{ 
              fontSize: '0.875rem',
              color: dataFetchStatus.phase === 'error' ? '#991b1b' : 
                     dataFetchStatus.phase === 'completed' ? '#166534' : '#1e40af',
              fontWeight: '500'
            }}>
            ...
            {dataFetchStatus.data && (
              <div style={{ 
                ...
                <span>Source: {dataFetchStatus.data.matches?.source || 'unknown'}</span>
            ...
            {dataFetchStatus.errors && dataFetchStatus.errors.length > 0 && (
              <div style={{ 
                ...
                {dataFetchStatus.errors.slice(0, 2).map((error, idx) => (
                  <div key={idx}>• {error.source}: {error.error}</div>
                ))}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                <h3 style={{ margin: 0, fontSize: '1.125rem', fontWeight: '600' }}>Data Collection Status</h3>
                <p style={{ margin: 0, fontSize: '0.875rem', opacity: 0.9 }}>Background collection monitoring</p>
              </div>
            </div>
            <button
          ...
          <div style={{ padding: '1.5rem' }}>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{
                backgroundColor: collectionStatus.status.isCollecting ? 'rgba(236, 201, 75, 0.1)' : 'rgba(72, 187, 120, 0.1)',
                border: '1px solid',
                borderColor: collectionStatus.status.isCollecting ? 'var(--accent-yellow)' : 'var(--accent-green)',
                borderRadius: '8px',
                padding: '12px',
                display: 'flex',
                alignItems: 'center',
                ...
                <div>
                  <div style={{ fontWeight: '500', color: 'var(--text-primary)' }}>
                    {collectionStatus.status.isCollecting ? 'Collection in Progress' : 'Collection Complete'}
                  </div>
                  <div style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                    {collectionStatus.status.isCollecting ? 
                      'Background collection is currently running...' : 
                      `Last completed: ${collectionStatus.status.lastCollection ? new Date(collectionStatus.status.lastCollection).toLocaleString() : 'Never'}`
                    }
                  </div>
                </div>
              </div>
            ...
            {collectionStatus.progress && (
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: '0.875rem', fontWeight: '500', marginBottom: '4px', color: 'var(--text-primary)' }}>
                  ...
                  {collectionStatus.progress.data && (
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                      Results: {collectionStatus.progress.data.matches || 0} matches, {collectionStatus.progress.data.playerStats || 0} player stats, {collectionStatus.progress.data.errors || 0} errors
                    </div>
                  )}
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '8px' }}>
                    Updated: {new Date(collectionStatus.progress.timestamp).toLocaleString()}
                  </div>
                </div>
              </div>
            )}
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '0.875rem', marginBottom: '4px', color: 'var(--text-primary)' }}>
                  <strong>Next Collection:</strong> {collectionStatus.status.nextCollection === 'Soon' ? 'Soon' : new Date(collectionStatus.status.nextCollection).toLocaleString()}
                </div>
                <div style={{ fontSize: '0.875rem', marginBottom: '4px', color: 'var(--text-primary)' }}>
                  <strong>Frequency:</strong> Every 30 minutes
                </div>
                <div style={{ fontSize: '0.875rem', color: 'var(--text-primary)' }}>
                  <strong>Cache Available:</strong> {collectionStatus.status.cacheAvailable ? '✅ Yes' : '❌ No'}
                </div>
              </div>
            </div>

            {/* Errors */}
            {collectionStatus.status.errors && collectionStatus.status.errors.length > 0 && (
              <div>
                <div style={{ 
                  display: 'flex', 
                  alignItems: 'center', 
                ...
                <div style={{
                  ...
                  {(showAllErrors ? collectionStatus.status.errors : collectionStatus.status.errors.slice(0, 3)).map((error, idx) => (
                    <div key={idx} style={{ fontSize: '0.875rem', marginBottom: '4px', color: 'var(--accent-red)' }}>
                      • {error.player} ({error.type}): {error.error}
                    </div>
                  ))}
                  {!showAllErrors && collectionStatus.status.errors.length > 3 && (
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '8px' }}>
                      + {collectionStatus.status.errors.length - 3} more errors (click "Show All" to view)
                    </div>
                  )}
                </div>
              </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span style={{ fontSize: '1.5rem' }}>🎓</span>
              <div>
                <h3 style={{ margin: 0, fontSize: '1.125rem', fontWeight: '600' }}>AI Coach Analysis</h3>
                <p style={{ margin: 0, fontSize: '0.875rem', opacity: 0.9 }}>Comprehensive portfolio review</p>
              </div>
            </div>
            <button
          ...
          <div style={{ padding: '1.5rem' }}>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '12px' }}>
                <div style={{
                  ...
                  alignItems: 'center',
                  padding: '1rem',
                  backgroundColor: coachingData.portfolio_grade.grade === 'A+' ? '#dcfce7' : 
                                 coachingData.portfolio_grade.grade === 'A' ? '#dbeafe' :
                                 coachingData.portfolio_grade.grade === 'B' ? '#fef3c7' : '#fee2e2',
                  borderRadius: '12px',
                  minWidth: '80px',
                  border: '2px solid',
                  borderColor: coachingData.portfolio_grade.grade === 'A+' ? '#166534' : 
                              coachingData.portfolio_grade.grade === 'A' ? '#1e40af' :
                              coachingData.portfolio_grade.grade === 'B' ? '#92400e' : '#991b1b'
                }}>
                  <span style={{
                    fontSize: '2rem',
                    fontWeight: 'bold',
                    color: coachingData.portfolio_grade.grade === 'A+' ? '#166534' : 
                           coachingData.portfolio_grade.grade === 'A' ? '#1e40af' :
                           coachingData.portfolio_grade.grade === 'B' ? '#92400e' : '#991b1b'
                  }}>
                ...
                <div>
                  <h4 style={{ margin: '0 0 8px 0', color: '#374151', fontSize: '1.125rem' }}>Portfolio Grade</h4>
            ...
            {/* Key Strengths */}
            {coachingData.key_strengths && coachingData.key_strengths.length > 0 && (
              <div style={{ marginBottom: '1.5rem' }}>
                <h4 style={{ 
                  color: '#059669', 
                  fontSize: '1rem', 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {coachingData.key_strengths.map((strength, index) => (
                    <div key={index} style={{
                      ...
                      <div style={{ fontWeight: '500', color: '#047857', marginBottom: '4px' }}>
                        {strength.area}
                      </div>
            ...
            {coachingData.areas_for_improvement && coachingData.areas_for_improvement.length > 0 && (
              <div style={{ marginBottom: '1.5rem' }}>
                <h4 style={{ 
                  color: '#dc2626', 
                  fontSize: '1rem', 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {coachingData.areas_for_improvement.slice(0, 5).map((improvement, index) => (
                    <div key={index} style={{
                      ...
                      <div style={{ 
                        ...
                        {improvement.priority && (
                          <span style={{
                            backgroundColor: improvement.priority === 'high' ? '#dc2626' : '#f59e0b',
                            color: 'white',
                            fontSize: '0.75rem',
                            padding: '2px 6px',
                            borderRadius: '4px',
                      ...
                      {improvement.action && (
                        <div style={{ fontSize: '0.75rem', color: '#b91c1c', fontStyle: 'italic' }}>
            ...
            {coachingData.meta_alignment && (
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                <div style={{
                  backgroundColor: coachingData.meta_alignment.status === 'excellent' ? '#ecfdf5' :
                                 coachingData.meta_alignment.status === 'good' ? '#fef3c7' : '#fef2f2',
                  border: '1px solid',
                  borderColor: coachingData.meta_alignment.status === 'excellent' ? '#d1fae5' :
                              coachingData.meta_alignment.status === 'good' ? '#fde68a' : '#fecaca',
                  borderRadius: '8px',
                  padding: '12px'
                }}>
                  <div style={{ 
                    ...
                    <div style={{
                      fontSize: '1.5rem',
                      fontWeight: 'bold',
                      color: coachingData.meta_alignment.status === 'excellent' ? '#047857' :
                             coachingData.meta_alignment.status === 'good' ? '#92400e' : '#991b1b'
                    }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>
                        {coachingData.meta_alignment.aligned_players} of {coachingData.meta_alignment.total_players} players align with current meta
                      </div>
                    </div>
                  </div>
            ...
            {/* Actionable Tips */}
            {coachingData.actionable_tips && coachingData.actionable_tips.length > 0 && (
              <div style={{ marginBottom: '1.5rem' }}>
                <h4 style={{ 
                  color: '#7c3aed', 
                  fontSize: '1rem', 
            ...
            {/* Player Predictions */}
            {coachingData.player_predictions && coachingData.player_predictions.length > 0 && (
                  ...
                  {coachingData.player_predictions.slice(0, 6).map((pred, index) => (
                    <div key={index} style={{
                      ...
                      <div style={{ 
                        ...
                        <div style={{
                          backgroundColor: pred.confidence > 0.8 ? '#10b981' : pred.confidence > 0.6 ? '#f59e0b' : '#ef4444',
                          color: 'white',
                          padding: '2px 8px',
                          borderRadius: '4px',
                          fontSize: '0.75rem',
                      ...
                      <div style={{ 
                        ...
                        <div style={{ textAlign: 'center' }}>
                          <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>Projected</div>
                          <div style={{ fontWeight: '600', color: '#1f2937' }}>
                            {pred.projected.toFixed(1)}
                          </div>
                        </div>
                        <div style={{ textAlign: 'center' }}>
                          <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>Ceiling</div>
                          <div style={{ fontWeight: '600', color: '#10b981' }}>
                            {pred.ceiling.toFixed(1)}
                          </div>
                        </div>
                        <div style={{ textAlign: 'center' }}>
                          <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>Floor</div>
