...
const PerformanceTest = ({ playerProjections, teamStacks }) => {
  ...
  const analyzeConstraints = () => {
    ...
    requiredPositions.forEach((pos) => {
      if (!positionCounts[pos] || positionCounts[pos] < 2) {
        missingPositions.push(
          `${pos} (${positionCounts[pos] || 0} available, need at least 2)`
        );
      }
    });
    ...
    const sortedBySalary = [...playerProjections].sort(
    ...
    return {
      ...
      canBuildLineups:
        missingPositions.length === 0 && teamsWithEnoughPlayers.length >= 2,
    };
  };
  ...
  const runPerformanceTest = async () => {
    ...
    if (!validation.canBuildLineups) {
      alert(
        `Cannot build lineups: Missing positions: ${validation.missingPositions.join(
          ", "
        )}, Teams with enough players: ${
          validation.teamsWithEnoughPlayers.length
        }`
      );
      return;
    }
    ...
    for (const lineupCount of testCases) {
      ...
      try {
        ...
        // Log optimizer state
        console.log(
          `Optimizer initialized with ${playerProjections.length} players and ${
            teamStacks?.length || 0
          } team stacks`
        );
        ...
        console.log(
          `Completed ${lineupCount} lineups in ${duration}ms, actual lineups generated: ${lineups.length}`
        );

        // Log lineup details if no lineups were generated
        if (lineups.length === 0) {
          console.error(
            "No lineups generated. Check optimizer constraints and player pool."
          );
        }
        ...
        if (actualLineups === 0) {
          console.warn(
            `WARNING: Generated 0 lineups for ${lineupCount} requested. Check optimizer constraints.`
          );
        }
        ...
        testResults.tests.push({
          ...
          avgPerLineup:
            actualLineups > 0 ? (duration / actualLineups).toFixed(1) : 0,
          lineupsPerSecond:
            actualLineups > 0
              ? (actualLineups / (duration / 1000)).toFixed(1)
              : "0",
        });
      } catch (error) {
    ...
    <div className="performance-test-container" style={{ padding: "20px" }}>
      ...
      <div
        style={{
          marginBottom: "20px",
          padding: "15px",
          backgroundColor: constraints.canBuildLineups
            ? "rgba(76, 175, 80, 0.1)"
            : "rgba(244, 67, 54, 0.1)",
          borderRadius: "5px",
          border: `1px solid ${
            constraints.canBuildLineups ? "#4caf50" : "#f44336"
          }`,
        }}
      >
        ...
        >
          <div>
            ...
            {Object.entries(constraints.positionCounts || {}).map(
              ([pos, count]) => (
          ...
          <div>
            ...
            {Object.entries(constraints.teamCounts || {})
              ...
              .map(([team, count]) => (
                <div
                  key={team}
                  style={{ color: count >= 4 ? "green" : "orange" }}
                >
        ...
        <div style={{ marginTop: "10px" }}>
          <p>
            <strong>Minimum possible lineup salary:</strong> $
            {constraints.minLineupSalary?.toFixed(0) || "N/A"}
          </p>
          <p>
            <strong>Salary cap:</strong> $50,000{" "}
            {constraints.salaryCapSufficient ? "✓" : "✗"}
          </p>
          <p>
            <strong>Can build lineups:</strong>{" "}
            {constraints.canBuildLineups ? "Yes ✓" : "No ✗"}
          </p>

          {constraints.missingPositions?.length > 0 && (
            <p style={{ color: "red" }}>
              <strong>Missing positions:</strong>{" "}
              {constraints.missingPositions.join(", ")}
            </p>
          )}

          {constraints.teamsWithEnoughPlayers?.length < 2 && (
            <p style={{ color: "red" }}>
              <strong>Not enough teams for stacking:</strong> Need at least 2
      ...
      <button
        ...
        disabled={
          isRunning || !playerProjections || playerProjections.length === 0
        }
              ...
              {results.tests.map((test, idx) => (
                <tr key={idx} style={{ borderBottom: "1px solid #eee" }}>
                  ...
                  <td style={{ padding: "10px" }}>
                    {test.error ? "-" : test.duration}
                  </td>
                  <td style={{ padding: "10px" }}>
                    {test.error ? "-" : test.avgPerLineup}
                  </td>
                  <td style={{ padding: "10px" }}>
                    {test.error ? "-" : test.lineupsPerSecond}
                  </td>
                  <td
                    style={{
                      padding: "10px",
                      color: test.error ? "red" : "green",
                    }}
                  >
              ...
              <>
                <p>
                  ...
                  {(
                    ...
                    results.tests.filter((t) => !t.error).length
                  ).toFixed(1)}{" "}
                  ms per lineup
                </p>
                <p>
