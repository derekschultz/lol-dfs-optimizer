...
class SimulatedAnnealingOptimizer extends AdvancedOptimizer {
  ...
  async runSimulatedAnnealing(count = 100) {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(solutions.length, count); i++) {
        ...
        const simProgress =
          80 + ((i + 1) / Math.min(solutions.length, count)) * 20;
      ...
      this.updateStatus(
        `Simulated annealing completed: ${simulatedResults.length} lineups`
      );
  ...
  async _performAnnealing(progressStart, progressEnd) {
    ...
    while (
      ...
      if (this.stagnationCount >= this.annealingConfig.maxStagnation) {
        ...
        this.stagnationCount = 0;
        this.debugLog(
          `Reheating temperature to ${this.currentTemperature} at iteration ${this.currentIteration}`
        );
      }
      ...
      if (this.currentIteration % 100 === 0) {
        const iterationProgress =
          (this.currentIteration / maxIterations) * progressRange;
        this.updateProgress(progressStart + iterationProgress, "annealing");
        this.updateStatus(
          `Annealing: T=${this.currentTemperature.toFixed(
            1
          )}, Best=${this.bestScore.toFixed(1)}`
        );
        await this.yieldToUI();
      }
    }
    ...
    this.debugLog(
      `Annealing completed after ${this.currentIteration} iterations`
    );
  }
  ...
  async _neighborSwapPlayer(lineup) {
    ...
    const currentPlayer = lineup.players.find(
    ...
    if (alternatives.length > 0) {
      ...
      const weights = alternatives.map((alt) => {
        const currentProj = this._safeParseFloat(
          currentPlayer.projectedPoints || 0,
          0
        );
      ...
      if (selectedAlt) {
        const playerIndex = lineup.players.findIndex(
          (p) => p.position === randomPosition
        );
  ...
  async _neighborSwapCaptain(lineup) {
    ...
    if (captainEligible.length > 0) {
      ...
      const bestCandidate = captainEligible.reduce((best, player) => {
        ...
        const playerCeiling =
          this._safeParseFloat(playerData.projectedPoints, 0) * 1.5; // Captain multiplier
        const bestCeiling =
          this._safeParseFloat(bestData.projectedPoints, 0) * 1.5;
      ...
      if (bestCandidate) {
        ...
        const playerIndex = lineup.players.findIndex(
          (p) => p.id === bestCandidate.id
        );
        const originalPlayer = this.playerPool.find(
          (p) => p.id === bestCandidate.id
        );
  ...
  async _neighborChangeStack(lineup) {
    ...
    if (stackableTeams.length > 0) {
      const targetTeam =
        stackableTeams[Math.floor(Math.random() * stackableTeams.length)];

      // Try to replace a non-stack player with a stack player
      const nonStackPlayers = lineup.players.filter(
        (p) => p.team !== targetTeam
      );
      const stackCandidates = this.playerPool.filter(
        (player) =>
      ...
      if (nonStackPlayers.length > 0 && stackCandidates.length > 0) {
        ...
        const replacement = stackCandidates.find(
  ...
  async _neighborOptimizeSalary(lineup) {
    ...
    if (remainingSalary > 1000) {
      ...
      const upgradeablePositions = lineup.players.filter((player) => {
        const upgrades = this.playerPool.filter(
          (p) =>
      ...
      if (upgradeablePositions.length > 0) {
        const targetPlayer =
          upgradeablePositions[
            Math.floor(Math.random() * upgradeablePositions.length)
          ];
        const upgrades = this.playerPool.filter(
          (p) =>
        ...
        const bestUpgrade = upgrades.reduce((best, upgrade) => {
          const upgradeValue =
            this._safeParseFloat(upgrade.projectedPoints, 0) /
            Math.max(1, this._safeParseFloat(upgrade.salary, 1) / 1000);
          const bestValue =
            this._safeParseFloat(best.projectedPoints, 0) /
  ...
  async _neighborBalanceExposure(lineup) {
    ...
    if (overexposedPlayers.length > 0) {
      const overexposedPlayer =
        overexposedPlayers[
          Math.floor(Math.random() * overexposedPlayers.length)
        ];
      ...
      const underexposedAlts = alternatives.filter((alt) => {
        ...
        const altCurrentExposure =
          totalLineups > 0 ? altCount / totalLineups : 0;
      ...
      if (underexposedAlts.length > 0) {
        const replacement =
          underexposedAlts[Math.floor(Math.random() * underexposedAlts.length)];
        ...
        if (overexposedPlayer.player.position === "CPT") {
          ...
          const playerIndex = lineup.players.findIndex(
            (p) => p.id === overexposedPlayer.player.id
          );
          lineup.players[playerIndex] = {
            id: replacement.id,
            name: replacement.name,
  ...
  _calculateConstraintSatisfactionScore(lineup) {
    let score = 0;
    const totalLineups =
      this.generatedLineups.length + this.existingLineups.length + 1;
    ...
    [lineup.cpt, ...lineup.players].forEach((player) => {
      ...
      if (exposure) {
        ...
        if (
          projectedExposure >= exposure.min &&
          projectedExposure <= exposure.max
        ) {
          score += 20;
        }
  ...
  _calculateSalaryEfficiencyScore(lineup) {
    ...
    } else if (salaryUsage >= 0.9) {
      return (salaryUsage - 0.9) * 100; // Up to 5 point bonus
    } else {
      return -(0.9 - salaryUsage) * 50; // Penalty for underusing salary
    }
  }
  ...
  _calculateAcceptanceRate() {
    ...
    const acceptedCount = this.acceptanceHistory.filter(
      (h) => h.accepted
    ).length;
  ...
  _getAnnealingSummary(results) {
    ...
    return {
      ...
      averageAnnealingScore:
        results.reduce((sum, r) => sum + (r.annealingScore || 0), 0) /
        results.length,
      acceptanceRate: this._calculateAcceptanceRate(),
      convergenceEfficiency: this._calculateConvergenceEfficiency(),
