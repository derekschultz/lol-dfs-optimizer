...
class HybridOptimizer {
  ...
  _initializePresets() {
    return {
      recommended: {
        name: 'Recommended',
        description: 'Smart algorithm selection based on your contest and constraints',
        algorithm: 'auto',
        config: {},
   ...
   */
  async initialize(playerPool, exposureSettings = {}, existingLineups = [], contestInfo = {}) {
    ...
    try {
      ...
      const constraintAnalysis = this._analyzeConstraints(exposureSettings, contestInfo);
      this.recommendedAlgorithm = this._selectOptimalAlgorithm(constraintAnalysis, contestInfo);
      ...
      await this._initializeOptimizers(playerPool, exposureSettings, existingLineups, constraintAnalysis);
  ...
  async optimize(count = 100, strategy = 'recommended', customConfig = {}) {
    if (!this.isInitialized) {
      throw new Error("Hybrid optimizer not initialized. Call initialize() first.");
    }
    ...
    try {
      ...
      this.updateStatus(`Optimization completed: ${results.lineups.length} lineups generated`);
  ...
  _analyzeConstraints(exposureSettings, contestInfo) {
    ...
    if (exposureSettings.players && exposureSettings.players.length > 0) {
      const activeConstraints = exposureSettings.players.filter(p => 
    ...
    if (exposureSettings.teams && exposureSettings.teams.length > 0) {
      const activeTeamConstraints = exposureSettings.teams.filter(t => 
      ...
      const stackConstraints = activeTeamConstraints.filter(t => t.stackSize !== undefined);
    ...
    if (exposureSettings.positions) {
      const positionConstraints = Object.keys(exposureSettings.positions).length;
  ...
  _selectOptimalAlgorithm(constraintAnalysis, contestInfo) {
    const { complexityScore, contestType, hasStackConstraints, constraintCount } = constraintAnalysis;
   ...
   */
  async _initializeOptimizers(playerPool, exposureSettings, existingLineups, constraintAnalysis) {
    const baseConfig = {
      salaryCap: 50000,
    ...
    await this.optimizers.monte_carlo.initialize(playerPool, exposureSettings, existingLineups);
    ...
    await this.optimizers.genetic.initialize(playerPool, exposureSettings, existingLineups);
    this.updateProgress(40 + progressStep * 2, "genetic_ready");
    
    // Initialize Simulated Annealing optimizer
    this.updateStatus("Initializing Simulated Annealing optimizer...");
    this.optimizers.simulated_annealing = new SimulatedAnnealingOptimizer(baseConfig);
    this._setupOptimizerCallbacks(this.optimizers.simulated_annealing);
    await this.optimizers.simulated_annealing.initialize(playerPool, exposureSettings, existingLineups);
  ...
  _getOptimalStrategyForContext() {
    const { complexityScore, contestType, hasStackConstraints, constraintCount } = this.constraintAnalysis || {};
  ...
  async _runHybridOptimization(count, strategy, customConfig) {
    const distribution = strategy.distribution || { monte_carlo: 0.6, genetic: 0.3, simulated_annealing: 0.1 };
    const results = [];
    ...
    for (const [algorithm, percentage] of Object.entries(distribution)) {
      ...
      this.updateStatus(`Running ${algorithm} optimization (${algorithmCount} lineups)...`);
      ...
      try {
        ...
        const algorithmConfig = {
          ...strategy.config,
          ...customConfig,
          [algorithm]: { ...strategy.config?.[algorithm], ...customConfig[algorithm] }
        };
        ...
        if (algorithm === 'monte_carlo') {
          ...
          algorithmResults = await optimizer.runGeneticOptimization(algorithmCount);
        } else if (algorithm === 'simulated_annealing') {
          algorithmResults = await optimizer.runSimulatedAnnealing(algorithmCount);
        }
      ...
      this.updateProgress(Math.min(95, totalProgress), `${algorithm}_completed`);
    }
  ...
  _combineResults(results, targetCount) {
    ...
    results.sort((a, b) => {
      const scoreA = a.nexusScore || a.roi || a.geneticFitness || a.annealingScore || 0;
      const scoreB = b.nexusScore || b.roi || b.geneticFitness || b.annealingScore || 0;
  ...
  _generateHybridSummary(results, distribution) {
    ...
    const avgROI = results.reduce((sum, r) => sum + (r.roi || 0), 0) / Math.max(1, results.length);
    const avgNexusScore = results.reduce((sum, r) => sum + (r.nexusScore || 0), 0) / Math.max(1, results.length);
  ...
  _calculateDiversityScore(lineups) {
    ...
    for (let i = 0; i < Math.min(lineups.length, 50); i++) { // Sample for performance
  ...
  _trackPerformance(strategyName, strategy, results) {
    ...
    if (this.performanceHistory.length > this.config.learning.historySize) {
      this.performanceHistory = this.performanceHistory.slice(-this.config.learning.historySize);
    }
  ...
  _updatePerformanceWeights() {
    if (!this.config.learning.adaptRecommendations || this.performanceHistory.length < 10) {
      return;
    }
    ...
    this.performanceHistory.forEach(record => {
      ...
      algorithmPerformance[record.algorithm].nexusScore.push(record.averageNexusScore);
      algorithmPerformance[record.algorithm].diversity.push(record.diversityScore);
    });
    
    // Update weights based on performance
    Object.entries(algorithmPerformance).forEach(([algorithm, metrics]) => {
      if (metrics.roi.length > 0) {
        const avgROI = metrics.roi.reduce((sum, r) => sum + r, 0) / metrics.roi.length;
        const avgNexus = metrics.nexusScore.reduce((sum, n) => sum + n, 0) / metrics.nexusScore.length;
        const avgDiversity = metrics.diversity.reduce((sum, d) => sum + d, 0) / metrics.diversity.length;
        
        // Combine metrics into performance score
        const performanceScore = (avgROI * 0.4) + (avgNexus * 0.4) + (avgDiversity * 100 * 0.2);
        
        // Update weight (slowly adapt)
        const currentWeight = this.config.performanceWeights[algorithm] || 1.0;
        const targetWeight = Math.max(0.5, Math.min(2.0, 1.0 + (performanceScore / 100)));
        this.config.performanceWeights[algorithm] = currentWeight * 0.9 + targetWeight * 0.1;
      }
    });
  }
  ...
  _getStrategyPerformance(strategyName) {
    const strategyRecords = this.performanceHistory.filter(r => r.strategy === strategyName);
    
    if (strategyRecords.length === 0) {
      return { usage: 0, averageROI: 0, averageNexusScore: 0, lastUsed: null };
    }
    
    const avgROI = strategyRecords.reduce((sum, r) => sum + r.averageROI, 0) / strategyRecords.length;
    const avgNexusScore = strategyRecords.reduce((sum, r) => sum + r.averageNexusScore, 0) / strategyRecords.length;
  ...
  _generateRecommendations(results) {
    ...
    if (diversityScore < 0.3) {
      recommendations.push({
        type: 'diversity',
        message: 'Consider using "Tournament" or "Contrarian" strategy for more lineup diversity',
        severity: 'warning'
      });
    }
    
    // Analyze constraint satisfaction
    if (this.constraintAnalysis?.constraintCount > 5 && results.summary?.algorithm !== 'simulated_annealing') {
      recommendations.push({
        type: 'constraints',
        message: 'Try "Constraint Optimizer" strategy for better exposure constraint handling',
        severity: 'info'
      });
    }
    ...
    if (avgROI < 0 && this.contestInfo?.type === 'gpp') {
      recommendations.push({
        type: 'performance',
        message: 'Consider "Contrarian" strategy for higher ceiling in tournaments',
        severity: 'suggestion'
      });
    }
  ...
  _savePerformanceHistory() {
    try {
      if (typeof localStorage !== 'undefined') {
        localStorage.setItem('lol_dfs_optimizer_performance', JSON.stringify(this.performanceHistory));
      }
    } catch (error) {
  ...
  getStats() {
    return {
      ...
      availableAlgorithms: Object.keys(this.optimizers).filter(key => this.optimizers[key]),
