...
class GeneticOptimizer extends AdvancedOptimizer {
    ...
    try {
      ...
      for (let generation = 0; generation < this.geneticConfig.generations; generation++) {
        ...
        if (this.stagnationCount >= this.geneticConfig.maxStagnation) {
          this.debugLog(`Restarting due to stagnation at generation ${generation}`);
        ...
        // Update progress
        const evolutionProgress = 20 + ((generation + 1) / this.geneticConfig.generations) * 60;
        this.updateProgress(evolutionProgress, "evolving");
        this.updateStatus(`Evolution: Generation ${generation + 1}/${this.geneticConfig.generations} (Best: ${currentBest.toFixed(1)})`);
      ...
      const selectedLineups = population.slice(0, count).map(ind => ind.lineup);
      ...
      this.updateStatus(`Genetic optimization completed: ${simulatedResults.length} lineups`);
      ...
      return {
        ...
        evolution: {
          ...
          finalDiversity: this._calculatePopulationDiversity(population.slice(0, count))
        }
      };
  ...
  async _initializePopulation() {
    ...
    for (let i = 0; i < populationSize; i++) {
      if (this.isCancelled) throw new Error("Population initialization cancelled");
    ...
    this.debugLog(`Created initial population of ${population.length} individuals`);
    return population;
  }
  ...
  _getInitializationStrategy(index, totalSize) {
    const strategies = [
      ...
      { name: 'stack_heavy', leverageMultiplier: 0.8, randomness: 0.2, stackBias: 'large' },
      { name: 'stack_light', leverageMultiplier: 0.6, randomness: 0.3, stackBias: 'small' }
    ];
    ...
    for (let batch = 0; batch < batches; batch++) {
      ...
      await Promise.all(
        population.slice(startIdx, endIdx).map(async (individual) => {
          individual.fitness = await this._calculateGeneticFitness(individual.lineup);
  ...
  async _calculateGeneticFitness(lineup) {
    ...
    let positionBonus = 0;
    const positionImpact = { MID: 2, ADC: 1.8, JNG: 1.5, TOP: 1.2, SUP: 1.0, TEAM: 0.8 };
  ...
  _calculateConstraintSatisfaction(lineup) {
    ...
    allPlayerIds.forEach(playerId => {
      ...
      if (exposure) {
        ...
        const currentExposure = totalLineups > 0 ? currentCount / totalLineups : 0;
  ...
  async _evolveGeneration(population) {
    ...
    const eliteCount = Math.floor(populationSize * this.geneticConfig.elitePercentage);
  ...
  _tournamentSelection(population) {
    const tournamentSize = Math.min(this.geneticConfig.tournamentSize, population.length);
    const tournament = [];
  ...
  async _crossover(parent1, parent2) {
    ...
    positions.forEach(position => {
      const p1Player = parent1.lineup.players.find(p => p.position === position);
      const p2Player = parent2.lineup.players.find(p => p.position === position);
    ...
    if (p1Team || p2Team) {
      const teamPlayer = p1Team && p2Team ? 
        (Math.random() < 0.5 ? p1Team : p2Team) :
        (p1Team || p2Team);
  ...
  async _mutateSwapPlayer(individual) {
    ...
    if (alternatives.length > 0) {
      const newPlayer = alternatives[Math.floor(Math.random() * alternatives.length)];
      const playerIndex = individual.players.findIndex(p => p.position === randomPosition);
  ...
  async _mutateSwapCaptain(individual) {
    ...
    if (captainEligible.length > 0) {
      const newCaptain = captainEligible[Math.floor(Math.random() * captainEligible.length)];
      ...
      const playerIndex = individual.players.findIndex(p => p.id === newCaptain.id);
      individual.players[playerIndex] = {
        id: oldCaptain.id,
        name: oldCaptain.name,
        position: oldCaptain.position === 'CPT' ? newCaptain.position : oldCaptain.position,
  ...
  async _mutateSwapTeamStack(individual) {
    ...
    const stackTeam = Object.entries(teamCounts)
      .sort((a, b) => b[1] - a[1])[0]?.[0];
    
    if (stackTeam) {
      // Try to replace a non-stack player with a stack player
      const nonStackPlayers = individual.players.filter(p => p.team !== stackTeam);
      const stackAlternatives = this.playerPool.filter(player => 
      ...
      if (nonStackPlayers.length > 0 && stackAlternatives.length > 0) {
        ...
        const replacement = stackAlternatives.find(p => p.position === targetPlayer.position);
        
        if (replacement) {
          const playerIndex = individual.players.findIndex(p => p.id === targetPlayer.id);
          individual.players[playerIndex] = {
            id: replacement.id,
    ...
    if (uniqueIds.size !== playerIds.length) {
      ...
      for (let i = 0; i < lineup.players.length; i++) {
        if (seenIds.has(lineup.players[i].id)) {
          const alternatives = this.playerPool.filter(p => 
            p.position === lineup.players[i].position && 
            !seenIds.has(p.id)
          );
          if (alternatives.length > 0) {
            const replacement = alternatives[Math.floor(Math.random() * alternatives.length)];
  ...
  _getGeneticSummary(results) {
    ...
    return {
      ...
      averageGeneticFitness: results.reduce((sum, r) => sum + (r.geneticFitness || 0), 0) / results.length,
      diversityScore: this._calculatePopulationDiversity(results.map(r => ({ lineup: r }))),
      evolutionEfficiency: this.fitnessHistory.length > 0 ? 
        (this.bestFitness - this.fitnessHistory[0].bestFitness) / this.fitnessHistory.length : 0
    };
  }
}
