...
class MLModelService {
    ...
    generatePlayerTrainingData(samples) {
        ...
        for (let i = 0; i < samples; i++) {
            ...
            const output = [adjustmentFactor, varianceFactor, ceilingFactor, floorFactor];
    ...
    generateLineupTrainingData(samples) {
        ...
        for (let i = 0; i < samples; i++) {
            ...
            const score = (balance * 0.2 + stacking * 0.3 + ownership * 0.2 + projectedPoints * 0.01) / 100;
    ...
    async predictPlayerPerformance(playerFeatures, player = {}) {
        ...
        const baseProjection = player.projectedPoints || player.projected_points || null;
        ...
        if (baseProjection && baseProjection > 5) {
            ...
            console.log(`🎯 ML Prediction for ${player.name}: Base=${baseProjection.toFixed(1)}, MLFactor=${mlFactor.toFixed(3)}, Adjustment=${mlAdjustment.toFixed(1)}, Final=${adjustedPoints.toFixed(1)}`);
    ...
    extractPlayerFeatures(player, context = {}) {
        ...
        const baseProjection = player.projectedPoints || player.projected_points || 12;
    ...
    extractLineupFeatures(lineup, context = {}) {
        ...
        const teamCounts = {};
        lineup.forEach(p => {
            teamCounts[p.team || 'unknown'] = (teamCounts[p.team || 'unknown'] || 0) + 1;
        });
        const maxStack = Math.max(...Object.values(teamCounts));
        features.push(maxStack / lineup.length);
        
        // Average ownership
        const avgOwnership = lineup.reduce((sum, p) => sum + (p.projected_ownership || 0.1), 0) / lineup.length;
        features.push(avgOwnership);
        
        // Projected points
        const projectedPoints = lineup.reduce((sum, p) => sum + (p.projected_points || 10), 0);
    ...
    extractRiskFeatures(portfolio, context = {}) {
        ...
        const maxPlayerExposure = Math.max(...Object.values(playerCounts)) / portfolio.length;
