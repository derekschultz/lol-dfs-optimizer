...
class RiotGamesAPI {
  ...
  async enforceRateLimit() {
    ...
    if (this.requestsInLastTwoMinutes.length >= 95) {
      // Leave buffer
  ...
  async getSummonerByName(
    ...
    try {
      ...
      if (error.response?.status === 404) {
        console.log(
          `🔍 Riot ID not found for ${summonerName}#${tag}, trying legacy endpoint...`
        );
        try {
          const baseUrl = `https://${this.regions[region]}`;
          const url = `${baseUrl}/lol/summoner/v4/summoners/by-name/${encodeURIComponent(
            summonerName
          )}`;
          ...
          if (
            legacyError.response?.status === 403 ||
            legacyError.response?.status === 404
          ) {
            const notFoundError = new Error(
              `Player not found: ${summonerName}#${tag}`
            );
