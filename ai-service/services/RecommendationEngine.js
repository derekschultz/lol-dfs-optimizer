...
class RecommendationEngine {
  ...
  analyzePortfolio(lineups, playerData) {
    ...
    lineups.forEach((lineup) => {
      ...
      if (lineup.cpt || lineup.captain) {
        ...
        analysis.playerExposure[playerKey] =
          (analysis.playerExposure[playerKey] || 0) + 1;
        analysis.teamExposure[captain.team] =
          (analysis.teamExposure[captain.team] || 0) + 1;
        analysis.positionExposure["CPT"] =
          (analysis.positionExposure["CPT"] || 0) + 1;
      }
      ...
      if (lineup.players) {
        lineup.players.forEach((player) => {
          ...
          analysis.playerExposure[playerKey] =
            (analysis.playerExposure[playerKey] || 0) + 1;
          analysis.teamExposure[player.team] =
            (analysis.teamExposure[player.team] || 0) + 1;
          analysis.positionExposure[player.position] =
            (analysis.positionExposure[player.position] || 0) + 1;
        });
      }
    });
    ...
    analysis.averageProjection =
      analysis.totalProjection / analysis.totalLineups;

    // Convert counts to percentages
    Object.keys(analysis.playerExposure).forEach((player) => {
      analysis.playerExposure[player] =
        (analysis.playerExposure[player] / analysis.totalLineups) * 100;
    });

    Object.keys(analysis.teamExposure).forEach((team) => {
      analysis.teamExposure[team] =
        (analysis.teamExposure[team] / analysis.totalLineups) * 100;
    });
  ...
  identifyOpportunities(portfolioAnalysis, playerData, contestData) {
    ...
    Object.entries(portfolioAnalysis.playerExposure).forEach(
      ([playerKey, exposure]) => {
    ...
    Object.entries(portfolioAnalysis.teamExposure).forEach(
      ([team, exposure]) => {
  ...
  async generateMetaRecommendations(playerData) {
    ...
    if (currentMeta.teamTrends["T1"] === "rising") {
      recommendations.push({
        ...
        category: "team_trend",
        message:
          "T1 showing strong recent form - consider increasing stack exposure",
        confidence: 0.78,
        priority: "medium",
        data: {
          team: "T1",
    ...
    switch (opportunity.type) {
      case "reduce_exposure":
        actionableData = {
          ...
          reduce_by: Math.min(
            25,
            opportunity.current_exposure - opportunity.recommended_exposure
          ),
        };
        break;
      case "increase_exposure":
        actionableData = {
          ...
          current_exposure: opportunity.current_exposure,
          increase_by:
            opportunity.recommended_exposure - opportunity.current_exposure,
        };
        break;
  ...
  generateMessage(opportunity) {
    switch (opportunity.type) {
      case "reduce_exposure":
        return `Currently at ${opportunity.current_exposure.toFixed(
          1
        )}% exposure. Recommend reducing to ${
          opportunity.recommended_exposure
        }% to minimize risk.`;
      case "increase_exposure":
        return `Currently at ${opportunity.current_exposure.toFixed(
          1
        )}% exposure. Consider increasing to ${
          opportunity.recommended_exposure
        }% for better upside.`;
      case "increase_team_stack":
        return `${
          opportunity.team
        } exposure at ${opportunity.current_exposure.toFixed(
          1
        )}%. Recommend increasing to ${
          opportunity.recommended_exposure
