...
class PlayerPredictor {
  ...
  async predictSinglePlayer(player, matchContext) {
    const cacheKey = `${player.name}_${player.team}_${JSON.stringify(matchContext)}`;
  ...
  extractFeatures(player, matchContext) {
    ...
    const baseFeatures = {
      ...
      // Match context
      opponent_strength: matchContext.opponent ? this.getTeamStrength(matchContext.opponent) : 0.5,
  ...
  getChampionPoolStrength(player) {
    ...
    return poolStrengths[player.name] || 0.80;
  }
  ...
  createPerformanceModel() {
    return {
      predict: (features) => {
        ...
        // Opponent adjustment
        prediction += (features.team_strength - features.opponent_strength) * 0.5;
