...
class DataCollector {
  ...
  async initialize() {
    try {
      ...
      if (!this.riotAPI) {
        console.warn('⚠️ No Riot API key provided - will use cached/fallback data');
        console.warn('💡 To enable live data collection, set RIOT_API_KEY environment variable');
        console.warn('💡 Get your API key from: https://developer.riotgames.com/');
      }
      ...
      if (this.riotAPI) {
        try {
          ...
          const testSummoner = await this.riotAPI.getSummonerByName('Faker', 'KR', 'KR1');
          ...
          if (error.status === 403) {
            ...
            console.error('   Please check your API key at: https://developer.riotgames.com/');
  ...
  async collectLatestData() {
    ...
    try {
      ...
      console.log(`📊 Results: ${results.matches?.matches?.length || results.matches?.total || 0} matches, ${results.players?.players?.length || results.players?.total || 0} players`);
      return results;
  ...
  async collectMatchData() {
    ...
    try {
      ...
      const collectMatches = async () => {
        ...
        console.log(`📥 Live data: ${liveData.players?.length || 0} players available`);
        ...
        const playersWithSummoners = liveData.players
          .filter(p => p.summonerName && p.region);
          
        console.log(`🎯 Found ${playersWithSummoners.length} players with summoner info`);
        
        for (const player of playersWithSummoners) {
          try {
            console.log(`🔍 Fetching matches for ${player.name} (${player.summonerName})...`);
            
            // Get summoner with timeout
            const summoner = await Promise.race([
              this.riotAPI.getSummonerByName(player.summonerName, player.region, player.tagLine),
              new Promise((_, reject) => setTimeout(() => reject(new Error('Summoner lookup timeout')), 5000))
            ]);
            ...
            const matchIds = await Promise.race([
              ...
              new Promise((_, reject) => setTimeout(() => reject(new Error('Match history timeout')), 5000))
            ]);
              ...
              for (const matchId of matchIds.slice(0, 2)) {
                try {
                  const matchData = await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Match processing timeout')), 8000))
                  ]);
                  
                  if (matchData) {
                    matches.push({
                      matchId: matchData.matchId,
                      date: new Date(matchData.gameCreation).toISOString().split('T')[0],
                      duration: Math.round(matchData.gameDuration / 60 * 10) / 10,
                      patch: matchData.championStats[0]?.stats?.gameVersion?.split('.').slice(0, 2).join('.') || 'Unknown',
                    ...
                    console.log(`✅ Added match ${matchId.slice(-8)} for ${player.name}`);
                  }
                } catch (matchError) {
                  console.warn(`⚠️ Failed to process match ${matchId.slice(-8)} for ${player.name}: ${matchError.message}`);
                }
              }
            }
            ...
            console.warn(`⚠️ Failed to fetch data for ${player.name}: ${error.message}`);
      ...
      console.error('❌ Match collection failed, using fallback:', error.message);
      return this.getFallbackMatchData();
    }
  }
    ...
    const mockMatches = [
      {
        ...
        patch: '14.24',
        players: [
          { name: 'Faker', team: 'T1', position: 'MID', kills: 4, deaths: 2, assists: 8, cs: 245, fantasyPoints: 28.45 },
          { name: 'Zeus', team: 'T1', position: 'TOP', kills: 2, deaths: 1, assists: 5, cs: 220, fantasyPoints: 21.20 }
        ]
      }
    ];
  ...
  async collectPlayerStats() {
    ...
    try {
      ...
      const collectStats = async () => {
        ...
        for (const player of playersWithSummoners) {
          try {
            ...
            const summoner = await Promise.race([
              this.riotAPI.getSummonerByName(player.summonerName, player.region, player.tagLine),
              new Promise((_, reject) => setTimeout(() => reject(new Error('Summoner timeout')), 4000))
            ]);
            
            // Get recent matches (just 3 for speed)
            const matchIds = await Promise.race([
              this.riotAPI.getMatchHistory(summoner.puuid, player.region, 3),
              new Promise((_, reject) => setTimeout(() => reject(new Error('Match history timeout')), 4000))
            ]);
            ...
            for (const matchId of matchIds.slice(0, 2)) { // Only 2 matches for speed
              try {
                const matchData = await Promise.race([
                  this.riotAPI.processMatchForStats(matchId, player.region),
                  new Promise((_, reject) => setTimeout(() => reject(new Error('Match processing timeout')), 6000))
                ]);
                
                if (matchData) {
                  const playerMatch = matchData.playerStats.find(p => p.puuid === summoner.puuid);
                  if (playerMatch) {
                ...
                console.warn(`⚠️ Failed to process match for ${player.name}: ${matchError.message}`);
              }
            }
            ...
            if (gamesPlayed > 0) {
              playerStats.push({
                ...
                stats: {
                  ...
                  avgFantasyPoints: recentForm.reduce((a, b) => a + b, 0) / recentForm.length
                }
              });
              console.log(`✅ Added stats for ${player.name} (${gamesPlayed} games)`);
            }
            ...
            console.warn(`⚠️ Failed to get stats for ${player.name}: ${error.message}`);
          }
        }
      ...
      console.error('❌ Player stats collection failed, using fallback:', error.message);
      return this.getFallbackPlayerStats();
    }
  }
