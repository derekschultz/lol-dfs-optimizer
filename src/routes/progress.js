...
router.get('/session/:sessionId', validateId, catchAsync(async (req, res) => {
...
router.get('/info/:sessionId', validateId, catchAsync(async (req, res) => {
  const progressService = req.app.get('services').progress;
  const { sessionId } = req.params;
...
router.post('/message/:sessionId', validateId, catchAsync(async (req, res) => {
  const progressService = req.app.get('services').progress;
  const { sessionId } = req.params;
  ...
  res.json({
    success: sent,
    message: sent ? 'Message sent successfully' : 'Failed to send message (session may be inactive)'
  });
}));

// Update progress for session
router.post('/update/:sessionId', validateId, catchAsync(async (req, res) => {
  const progressService = req.app.get('services').progress;
  const { sessionId } = req.params;
  ...
  const updated = progressService.updateProgress(sessionId, progress, message);

  res.json({
    success: updated,
    message: updated ? 'Progress updated successfully' : 'Failed to update progress (session may be inactive)'
  });
}));

// Update status for session
router.post('/status/:sessionId', validateId, catchAsync(async (req, res) => {
  const progressService = req.app.get('services').progress;
  const { sessionId } = req.params;
  ...
  res.json({
    success: updated,
    message: updated ? 'Status updated successfully' : 'Failed to update status (session may be inactive)'
  });
}));

// Close session
router.post('/close/:sessionId', validateId, catchAsync(async (req, res) => {
  const progressService = req.app.get('services').progress;
  const { sessionId } = req.params;
