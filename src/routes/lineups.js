...
const validateLineupData = (req, res, next) => {
  ...
  if (errors.length > 0) {
    return next(new AppError(`Lineup validation failed: ${errors.join(', ')}`, 400));
  }
...
router.get('/', catchAsync(async (req, res) => {
  ...
  res.json({
    success: true,
    data: lineups,
    message: `Retrieved ${lineups.length} lineups`
  });
}));
...
router.post('/', validateLineupData, catchAsync(async (req, res) => {
  const lineupService = req.app.get('services').lineup;
...
router.put('/:id', validateId, validateLineupData, catchAsync(async (req, res) => {
  const lineupService = req.app.get('services').lineup;
  const updatedLineup = await lineupService.updateLineup(req.params.id, req.body);
  res.json({
    success: true,
    data: updatedLineup,
...
router.delete('/bulk', catchAsync(async (req, res) => {
  ...
  if (!lineupIds || !Array.isArray(lineupIds) || lineupIds.length === 0) {
    throw new AppError('lineupIds array is required and must not be empty', 400);
  }
...
router.post('/dkentries', 
  ...
  catchAsync(async (req, res) => {
    ...
    try {
      ...
      const lineupService = req.app.get('services').lineup;
      const processedLineups = await lineupService.processFromDraftKingsEntries(csvData);

      if (processedLineups.length === 0) {
        throw new AppError('No valid lineup data found in the file after processing', 400);
      }
...
router.post('/generate', catchAsync(async (req, res) => {
  ...
  throw new AppError('Lineup generation not yet implemented in refactored API. Use optimization service.', 501);
}));
