...
const { validatePlayerData, validateId, validateFileUpload } = require('../middleware/validation');
...
router.get('/projections', catchAsync(async (req, res) => {
  const playerService = req.app.get('services').player;
  const players = await playerService.getAllPlayers();
  res.json({
    success: true,
...
router.post('/', validatePlayerData, catchAsync(async (req, res) => {
  const playerService = req.app.get('services').player;
...
router.put('/:id', validateId, validatePlayerData, catchAsync(async (req, res) => {
  const playerService = req.app.get('services').player;
...
router.delete('/bulk', catchAsync(async (req, res) => {
  const playerService = req.app.get('services').player;
  const { playerIds } = req.body;

  if (!playerIds || !Array.isArray(playerIds) || playerIds.length === 0) {
    throw new AppError('playerIds array is required and must not be empty', 400);
  }

  const result = await playerService.deletePlayers(playerIds);
  res.json({
    success: true,
    message: `Deleted ${playerIds.length} players successfully`,
    data: result
  });
}));

// GET /players/team/:team - Get players by team
router.get('/team/:team', catchAsync(async (req, res) => {
  const playerService = req.app.get('services').player;
...
router.get('/position/:position', catchAsync(async (req, res) => {
  const playerService = req.app.get('services').player;
  const players = await playerService.getPlayersByPosition(req.params.position);
  res.json({
    success: true,
    data: players
  });
...
router.post('/projections/upload', 
  ...
  catchAsync(async (req, res) => {
    const playerService = req.app.get('services').player;
    const playerRepository = req.app.get('services').playerRepository;
    ...
    try {
      ...
      if (processedPlayers.length === 0) {
        throw new AppError('No valid player data found in the file after processing', 400);
      }
...
router.get('/stats/teams', catchAsync(async (req, res) => {
  const playerService = req.app.get('services').player;
...
router.get('/stats/overview', catchAsync(async (req, res) => {
  const playerRepository = req.app.get('services').playerRepository;
  const [positionCounts, teamCounts, averageStats, totalCount] = await Promise.all([
