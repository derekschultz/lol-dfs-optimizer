...
router.put('/category/:category', catchAsync(async (req, res) => {
  const settingsService = req.app.get('services').settings;
  const { category } = req.params;
  const categorySettings = req.body;

  const updatedSettings = await settingsService.updateSettingCategory(category, categorySettings);
...
// Get settings schema
router.get('/schema', catchAsync(async (req, res) => {
  const settingsService = req.app.get('services').settings;
...
router.get('/export', catchAsync(async (req, res) => {
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="optimizer-settings.json"');
  res.json(exportData);
}));
...
router.get('/defaults', catchAsync(async (req, res) => {
  const settingsService = req.app.get('services').settings;
