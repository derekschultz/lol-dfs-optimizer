...
router.post(
  ...
  catchAsync(async (req, res) => {
    ...
    const result = await fileProcessingService.parseTeamStacksCSV(
      req.file.path
    );
...
router.post(
  ...
  catchAsync(async (req, res) => {
    ...
    const result = await fileProcessingService.parseDraftKingsCSV(
      req.file.path
    );
...
router.post(
  ...
  catchAsync(async (req, res) => {
    ...
    const csvContent =
      await fileProcessingService.generateTeamStacksCSV(stacks);

    res.setHeader("Content-Type", "text/csv");
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="team-stacks.csv"'
    );
    res.send(csvContent);
  })
);
...
router.post(
  ...
  catchAsync(async (req, res) => {
    ...
    const csvContent =
      await fileProcessingService.generateDraftKingsCSV(lineups);

    res.setHeader("Content-Type", "text/csv");
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="draftkings-lineups.csv"'
    );
    res.send(csvContent);
  })
);
...
router.post(
  ...
  catchAsync(async (req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${filename || "data"}.json"`
    );
