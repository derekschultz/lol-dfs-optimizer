...
const validateTeamStackData = (req, res, next) => {
  ...
  if (!team || typeof team !== 'string' || team.length < 1 || team.length > 50) {
  ...
  if (stackPlus !== undefined && (typeof stackPlus !== 'number' || isNaN(stackPlus))) {
    errors.push('Stack+ rating must be a valid number');
  }

  if (errors.length > 0) {
    return next(new AppError(`Team stack validation failed: ${errors.join(', ')}`, 400));
  }
...
router.get('/stacks', catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
...
router.get('/stacks/raw', catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
  const stacks = await teamStackService.getAllStacks();
  res.json({
    success: true,
...
router.get('/stacks/:id', validateId, catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
...
router.get('/:team/stacks', catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
...
router.post('/stacks', validateTeamStackData, catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
...
router.put('/stacks/:id', validateId, validateTeamStackData, catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
  const updatedStack = await teamStackService.updateStack(req.params.id, req.body);
  res.json({
    success: true,
...
router.delete('/stacks/:id', validateId, catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
...
router.delete('/stacks/bulk', catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
  const { stackIds } = req.body;

  if (!stackIds || !Array.isArray(stackIds) || stackIds.length === 0) {
    throw new AppError('stackIds array is required and must not be empty', 400);
  }
...
router.post('/stacks/search', catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
...
router.get('/stacks/stats/overview', catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
...
router.get('/stacks/top/:limit?', catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
...
router.get('/stacks/tiers', catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
...
router.post('/stacks/export', catchAsync(async (req, res) => {
  const teamStackService = req.app.get('services').teamStack;
