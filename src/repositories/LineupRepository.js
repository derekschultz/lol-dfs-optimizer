...
class LineupRepository {
  ...
  async search(filters = {}) {
    ...
    if (filters.name) {
      results = results.filter(
        (lineup) =>
          lineup.name &&
          lineup.name.toLowerCase().includes(filters.name.toLowerCase())
      );
    }
    ...
    if (filters.minNexusScore) {
      results = results.filter(
        (lineup) => (lineup.nexusScore || 0) >= filters.minNexusScore
      );
    }

    if (filters.maxNexusScore) {
      results = results.filter(
        (lineup) => (lineup.nexusScore || 0) <= filters.maxNexusScore
      );
    }
  ...
  calculateTotalProjection(lineup) {
    ...
    const playersProjection =
      lineup.players?.reduce((sum, p) => sum + (p.projectedPoints || 0), 0) ||
      0;
  ...
  getTeamComposition(lineup) {
    ...
    if (lineup.players) {
      lineup.players.forEach((player) => {
        if (player.team) {
          teamComposition[player.team] =
            (teamComposition[player.team] || 0) + 1;
        }
      });
    }
  ...
  async getAverageStats() {
    ...
    const totals = this.lineups.reduce(
      (acc, lineup) => ({
  ...
  async getPositionExposure() {
    ...
    this.lineups.forEach((lineup) => {
      ...
      if (lineup.cpt?.position) {
        positionCounts[lineup.cpt.position] =
          (positionCounts[lineup.cpt.position] || 0) + 1;
      }
    ...
    Object.keys(positionCounts).forEach((position) => {
      positionExposure[position] =
        totalLineups > 0
          ? parseFloat(
              ((positionCounts[position] / totalLineups) * 100).toFixed(2)
            )
          : 0;
    });
