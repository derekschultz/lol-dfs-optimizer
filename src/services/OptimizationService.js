...
class OptimizationService {
    ...
    try {
      ...
      try {
        switch (algorithm) {
          ...
          default:
            throw new AppError(`Unknown optimization algorithm: ${algorithm}`, 400);
        }
  ...
  async _runHybridOptimization(optimizer, options) {
    ...
    optimizer.initialize({
      ...
      teamStacks: options.teamStacks || [],
      exposureLimits: options.exposureLimits || {},
      constraints: options.constraints || {}
    });

    // Generate lineups with hybrid approach
    const lineups = await optimizer.optimize({
      numLineups: options.numLineups,
  ...
  cancelOptimization(optimizationId) {
    ...
    throw new AppError(`Cannot cancel optimization with status: ${optimization.status}`, 400);
  }

  /**
   * Clean up completed optimizations
   */
  cleanupOptimizations(maxAge = 3600000) { // 1 hour default
    const now = Date.now();
    const cleaned = [];
  ...
  async initializeHybridOptimizer(options = {}) {
    ...
    try {
      ...
      const allPlayers = players.length > 0 ? players : await this.playerRepository.getAll();
      ...
      this.activeOptimizations.set(initId, {
        ...
        status: 'initialized',
        optimizer,
        config: { players: allPlayers, teamStacks: allStacks, constraints, strategies },
        createdAt: Date.now()
      });
      ...
      throw new AppError(`Hybrid optimizer initialization failed: ${error.message}`, 500);
    }
  }
  ...
  async generateHybridLineups(initId, options = {}) {
    ...
    if (!optimization || optimization.type !== 'hybrid_init') {
      throw new AppError('Invalid initialization ID or optimizer not initialized', 400);
    }
  ...
  async getOptimizerStats() {
    ...
    if (allLineups.length > 0) {
      const totalScore = allLineups.reduce((sum, lineup) => sum + (lineup.projectedScore || 0), 0);
   ...
   */
  async _runMonteCarloSimulation(lineups, iterations, fieldSize, payoutStructure) {
    ...
    results.forEach(result => {
      ...
      result.averageScore = result.simulationScores.reduce((a, b) => a + b, 0) / result.simulationScores.length;
      result.scoreVariance = this._calculateVariance(result.simulationScores);
      result.roi = payoutStructure ? ((result.totalPayout / iterations) - 1) * 100 : 0;
    });
  ...
  _calculateAverageScore(lineups) {
    ...
    const total = lineups.reduce((sum, lineup) => sum + (lineup.projectedScore || 0), 0);
  ...
  _calculateExposureStats(lineups) {
    ...
    const exposureStats = {
      players: Object.entries(playerExposure).map(([name, count]) => ({
        name,
        exposure: (count / totalLineups) * 100
      })).sort((a, b) => b.exposure - a.exposure),
      teams: Object.entries(teamExposure).map(([team, count]) => ({
        team,
        exposure: (count / totalLineups) * 100
      })).sort((a, b) => b.exposure - a.exposure)
    };
  ...
  _calculateAverageWinRate(results) {
    ...
    const total = results.reduce((sum, result) => sum + (result.winRate || 0), 0);
  ...
  getActiveOptimizations() {
    return Array.from(this.activeOptimizations.entries()).map(([id, optimization]) => ({
      id,
      ...optimization
    }));
  }
