...
function getMatchDominanceFactor(stackPlusAllWins, stackPlusAllLosses) {
  ...
  if (stackPlusAllWins >= 200) return 1.1; // Elite sweep teams
  ...
  if (stackPlusAllLosses < 5) return 1.0; // Expected pattern
...
function getStackModifier(teamStack, contestType = null) {
  ...
  return (
    getStackRatingModifier(teamStack.stackPlus) *
    getMatchDominanceFactor(
      teamStack.stackPlusAllWins,
      teamStack.stackPlusAllLosses
    )
  );
...
function identifyPrimaryStack(lineup) {
  ...
  const primaryTeam = Object.entries(teamCounts).sort(
    ([, a], [, b]) => b - a
  )[0][0];
